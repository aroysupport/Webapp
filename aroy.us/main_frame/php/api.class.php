...
class api {
	...
	public function CampaignImage() {
		...
		$BrandCampaign = $this -> getBrandCampaign();
		$temp = $this -> get("getAllCampaignImages");
		$json =$temp["GetAllCampaignImagesResult"];
		//echo "<pre>";
		//$timer = microtime(true);
		//echo "ProductImage ";
		
		function CampaignImage_array_method2($item, $key, $json) {
			$arrTemp = array();
			$api = new api();			
			//echo "		Product: $item ";
			//echo "<br>";
			foreach ($json as $a => $b) {
				if ($b['CampaignID'] == $item) {
					//echo "			|Pushed		This.ProductID: " . $b['ProductID'] . "	|ProductID: " . $item . "	|image: " . $b['ImageID'] . "<br>";
					array_push($arrTemp, $b['ImageID']);
				}
			}
			...
			if (!empty($arrTemp)) {
				$api -> setCampaignImage($item, $arrTemp);
			}
		}

		function CampaignImage_array_method1($item, $key,$json) {
			//echo "Brand -> $key<br>";
			if(is_array($item)){
				array_walk($item, 'CampaignImage_array_method2',$json);	
			}
		}

		array_walk($BrandCampaign,'CampaignImage_array_method1',$json);
		//echo "<br>-------------------------------------<br>";
		//printf("Result: %.5f sec<br>", microtime(true) - $timer);
	}
	...
	public function campaignTemplate($data) {
		foreach ($data as $k => $v) {
			foreach ($v as $a => $key) {
				$str = '<tr data-toggle="collapse" data-target=".campaign'.$key['CampaignID'].'" class="accordion-toggle">' . 
				'<td><input type="checkbox" id="' . $key['CampaignID'] . '">' . 
				'</td><td><label for="' . $key['CampaignID'] . '">' . $key['Name']. '</label></td>' . 
				'</tr>'.
				'<tr><td class="hidden-row"></td><td class="hidden-row"><div class="accordian-body collapse campaign'.$key['CampaignID'].'">'.
				
				'<ul><li>Start Date: ' . $key['StartDate'] . 
				'</li><li>End Date:' . $key['EndDate'] . 
				'</li><li>Location: ' . $key['Market'] . 
				'</li><li>Clientele:' . 
				'</li><li>^^Campaign Identifier' . 
				'</li><li>Campaign Message' . 
				'<ul><li>Title - ' . $key['MessageTitle'] . 
				'</li><li>Name - ' . $key['Name'] . 
				'</li><li>Message - ' . $key['Message'] . 
				'</li></ul></li><li>QRCode Location: ' . $key['QRCodeLocation'] . 
	...
	public function brandTemplate($data) {
		...
		foreach ($data as $k => $v) {
			foreach ($v as $a => $key) {
				$str = "<tr value='" . $key['BrandID'] . "'><td>" . $key['Name'] . 
				"</td>" . "<td><a class='brandEdit edit Newlightbox' href='#popup2'>Edit</a></td>" . 
				"<input type='hidden' name='Name' value='" . $key['Name'] . "'class='brandPost'/>" . 
				"<input type='hidden' name='BrandID' value='" . $key['BrandID'] . "' class='brandID'/>" . 
				"<input type='hidden' name='Description' value='" . $key['Description'] . "' class='Description'/>" . 
				"<input type='hidden' name='BrandPhilosophy' value='" . $key['BrandPhilosophy'] . "' class='BrandPhilosophy'/>" . 
				"<input type='hidden' name='MicroBlogs' value='" . $key['MicroBlogs'] . "' class='MicroBlogs'/>" . 
				"<input type='hidden' name='BrandInfo' value='" . $key['BrandInfo'] . "' class='BrandInfo'/>" . "</tr>";
				echo $str;
			}
		}
		//echo "</select>";
	...
	public function getCurrentCampaign($brandID) {
		$arr = array();
		$CampaignID = $this -> getBrandCampaign();
	...
	public function setCampaignImage($campaignD, $imageID) {
		//echo "pushed ".print_r($imageID);
		//array_push($this -> productImage[$productID], $imageID);
		//$this -> productImage[$productID] = $imageID;
		$_SESSION['campaignImage'][$campaignD] = $imageID;
	}

	public function getCampaignImage() {
		return $_SESSION['campaignImage'];
	}
