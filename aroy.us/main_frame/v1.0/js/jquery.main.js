...
$(function() {
	...
	initData();
	addBrandRow();
	addCollectionRow("#new-product", ".productEdit", "#ProductNameBtn", "#productName", ".product-table");
	//---------init style--------
	init();
	//---------init style-------
	initUploadFiles();
...
function init() {
	$(document).ajaxStop(function() {
		//$('#wrapper').height($(this).height());
		//$('.tab-data, .library-block,  .right-col').height($(this).height() - 98);
		//$(".image-container, .inner-holder").height($(this).height() - 220);
		initDatepicker();
		initCustomForms();
		initTabs();
		initAddClasses();
...
function getCurrentBrandInfo() {
	$(".brand-table tr").each(function() {
		var id = $(this).find("input.brandID").val();
		if (id == currentBrandID) {
			return $(this).find("input.brandPost").val();
		}
	});
}

function getCurrentBrandInfo() {
	$(".instricition-box textarea[name=BrandPhilosophy]").val();
	$(".instricition-box textarea[name=Description]").val();
	$(".instricition-box input[name=BrandInfo]").val();
}
...
function onChangeSelect() {
	$('.accountBrand').on('change', function() {
		...
		updateBrandDropDown();
		getAllBrandInfo();
		getTest(".library-table", "campaign", "manager",currentBrandID);
		getTest(".product-table", "product", "manager", currentBrandID);
		getTest(".product", "product", null, currentBrandID);
		getTest(".campaign", "campaign", null, currentBrandID);
		imageload(currentBrandID);
		//updateBrandDropDown();
	});
	$('.accountID').on('change', function() {
		currentAccountID = $(this).val();
		getTest(".collection", "collection");
		getTest(".accountBrand", "brand");
		getTest(".brand-table", "brand", "manager");
	});
}
 ...
 */
function getTest(element, method, type, BrandID) {
	//var url = "http://localhost:8888/php/Brands.php?method="+method;
	var url = "http://" + global_domain + "/php/Brands.php?method=" + method + "&id=" + cookieID + "&accountID=" + currentAccountID + "&brand=" + BrandID;
	...
	$.ajax({
		...
		success : function(data) {
			console.log("element = " + element + ", method = " + method + ",type =  " + type);
					...
					if (data.length > 1) {
						...
						$.when($(element).append(data)).done(function() {
							...
							if (method == "product") {
								console.log("-------product------");
								lightboxReload();
								//getTest(".newBrand", "brand");
								$('.account-form .accountBrand').find('option').clone().appendTo('.newBrand');
								updateBrandDropDown();
								//getTest(".newCollection", "collection");
								$('.form-row .collection').find('option').clone().appendTo('.newCollection');
								$(document).find(".newBrand").removeClass("newBrand").addClass("brand");
								$(document).find(".newCollection").removeClass("newCollection").addClass("collection");
								//getProductCollection();
								
							} else if (method == "brand") {
								console.log("-------Brand------");
								currentBrandID = $(".accountBrand").val();
								console.log("currentAccountID", currentBrandID);
								getAllBrandInfo();
...
function getProductAndCampaign(currentBrandID) {
	getTest(".product", "product", null, currentBrandID);
	getTest(".campaign", "campaign", null, currentBrandID);
	getTest(".product-table", "product", "manager", currentBrandID);
	getTest(".library-table", "campaign", "manager", currentBrandID);
...
function getAllBrandInfo() {
	$(".brand-table tr:gt(0)").each(function() {
		console.log("check currentBranID = ", currentBrandID);
		...
		if ($(this).attr("value") == currentBrandID) {
			...
			$(".brand-block2 form").find('textarea[name=BrandPhilosophy]').val($(this).find("input.BrandPhilosophy").val());
			$(".brand-block2 form").find('input[name=BrandInfo]').val($(this).find("input.BrandInfo").val());
			$(".brand-block2 form").find('textarea[name=Description]').val($(this).find("input.Description").val());
			...
			$(".micro-block tr:eq(1)").find('.handle').val(trimed);
				...
				$(".micro-block tr:eq(1)").find('.handle').val(MicroBlogs);
			}
			/*$(".brand-block2 form").find('textarea[name=BrandPhilosophy]').val($(this).find("input.BrandPhilosophy").val());
			 $(".brand-block2 form").find('input[name=BrandInfo]').val($(this).find("input.BrandInfo").val());
			 $(".brand-block2 form").find('textarea[name=Description]').val($(this).find("input.Description").val());*/
...
function initData() {
	//var url = "http://localhost:8888/php/Brands.php?method=check";
	var url = "http://" + global_domain + "/php/Brands.php?method=init&id=" + cookieID;
	str = "";
	console.log("URL: ", url);
	$.ajax({
		type : "GET",
...
function createCampaignImage(){
	...
	$(checkCampaign).each(function(){
		...
		$(selectedImage).each(function(){
			var  image=$(this);
			console.log("CampaignID = ",campaign.attr("id"));
			console.log("ImageID = ",image.find("input#ImageID").val());
...
function checkStatus() {
	//var url = "http://localhost:8888/php/Brands.php?method=check";
	var url = "http://" + global_domain + "/php/Brands.php?method=check&id=" + cookieID;
	str = "";
	console.log("URL: ", url);
	$.ajax({
		type : "GET",
		...
		if ( typeof json === "object") {
			...
			//getTest(".product-table", "product", "manager" );
			//getTest(".library-table", "campaign", "manager");
		} else {
...
function test() {
	//var url = "http://localhost:8888/php/Brands.php?method=check";
	var url = "http://" + global_domain + "/php/Brands.php?method=destory&id=" + cookieID;
	console.log("URL: ", url);
	$.ajax({
		type : "GET",
		url : url
...
function ConvertFormToJSON(form) {
	...
	jQuery.each(array, function() {
		...
		if (this.name in json) {
			console.log("	--------------Stored-------------------Json= " + JSON.stringify(json));
			temp[i] = json;
			...
			console.log("Name" + this.name + " : Value =" + this.value);
			json[this.name] = this.value;
			console.log("json[" + this.name + "] : " + json[this.name]);
		}
	});
	temp[i] = json;
...
function ConvertFormToJSONExtra(form, options) {
	var array = jQuery(form).serializeArray();
	var json = {};
	var temp = {};
	var i = 0;
	...
	jQuery.each(array, function() {
		...
		if (this.name in json) {
			...
			json[this.name] = this.value;
			/*if(options == "brand"){
			 json["BrandPhilosophy"] = $(".brand-block2 form").find('textarea[name=BrandPhilosophy]').val();
			 json["BrandInfo"] = $(".brand-block2 form").find('input[name=BrandInfo]').val();
			 json["Description"] = $(".brand-block2 form").find('input[name=Description]').val();
			 var MicroBlogs ="";
			 switch($(".micro-block tr:eq(1)").find('.blog_type').val()){
			 case "twitter":
			 MicroBlogs = "https://twitter.com/";
			 case "weibo":
			 MicroBlogs = "http://weibo.com/";
			 }
			 if($(".micro-block tr:eq(1)").find('.handle').val()!=""){
			 json["MicroBlogs"] = MicroBlogs + $(".micro-block tr:eq(1)").find('.handle').val();
			 }
			 }*/
			console.log("json[" + this.name + "] : " + json[this.name]);
		}
...
function postBrand() {
	jQuery('.brand-block form.Post, .brand-block2 form, .micro-block').bind('submit', function(event) {
		event.preventDefault();
		var form = ".brand-block form.Post";
		console.log("-----check current form = ", this);
		//console.log(" ")
		//if (this == $(".brand-block2 form") || this == $(".micro-block")) {
		$(".brand-table tr").each(function() {
			console.log("check currentBranID = ", currentBrandID);
			if ($(this).attr("value") == currentBrandID) {
				//if ($(".brand-block2 form").find('textarea[name=BrandPhilosophy]').val())
					...
					if ($(".micro-block tr:eq(1)").find('.handle').val() != "") {
						$(this).find("input.MicroBlogs").val(MicroBlogs + $(".micro-block tr:eq(1)").find('.handle').val());
					}
			}
		});
		//}
		//var json = ConvertFormToJSON(form);
		var json = ConvertFormToJSONExtra(form, "brand");
		console.log(json);
		$.ajax({
		type : "POST",
		url : "http://" + global_domain + "/php/Brands.php?method=postBrand&id=" + cookieID + "&accountID=" + currentAccountID,
		//url : "http://localhost:8888/php/index.php?method=postBrand&ID=" + getURLParameter("ID"),
		data : json
...
function postProduct() {
	jQuery('.product-block form.Post').bind('submit', function(event) {
		event.preventDefault();
		var form = this;
		//var json = ConvertFormToJSON(form);
		var json = ConvertFormToJSON(form);
		...
		$.ajax({
			type : "POST",
			url : "http://" + global_domain + "/php/Brands.php?method=postProduct&id=" + cookieID,
			//url : "http://localhost:8888/php/index.php?method=postProduct&ID=" + getURLParameter("ID"),
			data : json
		}).done(function(respones) {
			/*if(respones == "success"){
			 alert("success!");
...
function postCampaign() {
	jQuery('form.compaign-form').bind('submit', function(event) {
		event.preventDefault();
		var form = this;
		//var json = ConvertFormToJSON(form);
		//console.log("--------------------------------------------Campaign");
		var json = ConvertFormToJSON(form);
		//json[0]["QRCodeLocation"] = file.url;
		console.log(json);
		$.ajax({
			type : "POST",
			url : "http://" + global_domain + "/php/Brands.php?method=postCampaign&id=" + cookieID + "&brand=" + currentBrandID,
			//url : "http://localhost:8888/php/index.php?method=campaign",
			data : json
		}).done(function(respones) {
...
function imagePost(file) {
	...
	arr["Name"] = url.substring(url.indexOf("test.aroy/") + 10, url.length);
	arr["ProductID"] = $("#upload_Product").val();
	console.log("image json = ", arr);
	var temp = {};
	temp[0] = arr;
	var imagePost = $.ajax({
		type : "POST",
		url : "http://" + global_domain + "/php/Brands.php?method=postImage&id=" + cookieID,
		//url : "http://localhost:8888/php/index.php?method=campaign",
		data : temp
	}).done(function(respones) {
...
function postCampaignImage() {
	jQuery('form.content-form').bind('submit', function(event) {
		event.preventDefault();
		var json = createCampaignImage();
		console.log(json);
		$.ajax({
			type : "POST",
			url : "http://" + global_domain + "/php/Brands.php?method=postCampaignImage&id=" + cookieID,
			//url : "http://localhost:8888/php/index.php?method=campaign",
			data : json
		}).done(function(respones) {
			alert("Done!");
...
function addBrandRow() {
	...
	$(document).on("click", ".brandEdit", function() {
		...
		update = $(this).parent().prev();
		tr = $(this).parent().parent();
		$("#BrandName").val(update.html());
	});

	$(document).on("click", "#BrandNameBtn", function() {
		if (clicked) {
			$(".brand-table").append('<tr><td>' + $("#BrandName").val() + '</td><td><a class="brandEdit edit Newlightbox" href="#popup2">Edit</a></td><input type="hidden" class="brandPost" name="Name" value="' + $("#BrandName").val() + '"/></tr>');
			lightboxReload();

		} else if (update != null) {
			console.log("updated");
			update.html($("#BrandName").val());
			console.log("before update value:", $(tr).find(".brandPost").val());
			console.log("input:", $("#BrandName").val());
			$(tr).find(".brandPost").val($("#BrandName").val());
			console.log("updated value:", $(tr).find(".brandPost").val());
		} else {
			console.log("problem ");
		}
...
function addCollectionRow(addBtn, editBtn, popupBtn, popupInput, table) {
	var clicked = false;
	var update, tr;
	$(document).on("click", ".cancelReset, #fancybox-close,#fancybox-overlay", function() {
		console.log("closed");
	...
	$(document).on("click", editBtn, function() {
		...
		console.log($(this).parent().prev().html());
		update = $(this).parent().prev();
		editTr = $(this).parent().parent();
		$(popupInput).val(update.html());
	});
	$(document).on("click", popupBtn, function() {
		if (clicked) {
			$(table).append('<tr><td>' + $(popupInput).val() + '</td><td><a class="productEdit edit Newlightbox" href="#popup1">Edit</a></td>' + '<td id="brandSize">' + '<select class="newBrand" name="BrandID">' + '<option>Select Brand</option></select></td><td><select class="newCollection" name="CollectionID">' + '<option>Select Collection</option></select></td><input type="hidden" class="productCollection" name="Name" value="' + $(popupInput).val() + '"/></tr>');

			$('.account-form .accountBrand').find('option').clone().appendTo('.newBrand');
			$('.form-row .collection').find('option').clone().appendTo('.newCollection');
			$(tr).find(".newBrand").removeClass("newBrand").addClass("brand");

			$(tr).find(".newCollection").removeClass("newCollection").addClass("collection");

			initCustomForms();
			lightboxReload();
		} else if (update != null) {
			console.log("updated");
			update.html($(popupInput).val());

			console.log("before update value:", $(editTr).find(".productCollection").val());
			console.log("input:", $(popupInput).val());
			$(editTr).find(".productCollection").val($(popupInput).val());
			console.log("updated value:", $(editTr).find(".productCollection").val());
		} else {
			console.log("problem ");
		}
		$(popupInput).val("");
...
;( function($) {
		...
		var slideHiddenClass = 'js-slide-hidden'; ( function() {
