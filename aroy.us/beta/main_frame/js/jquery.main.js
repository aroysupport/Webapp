...
var global_domain = "beta.aroy.us/";
var currentPage;
...
$(function() {
	...
	/*$("a").click(function(event) {
	 ...
	 console.log("href =", event.target.hash);
	 currentPage = event.target.hash;
	 MainControl();
	 });*/
	/*------------bind-----------*/
	addRow("#addBrand", ".brandEdit", "#BrandNameBtn", "#BrandName", ".brand-table", ".brandPost", true);
	addRow("#new-product", ".productEdit", "#ProductNameBtn", "#productName", ".product-table", ".productCollection", false);
	addRow("#edit-brand-philosophy", "#edit-brand-philosophy", "#philosophySave", "#BrandPhilosophy", ".content-brand-philosophy", ".brand-block2", false);
	addRow("#edit-brand-philosophy", "#edit-brand-info", "#infoSave", "#BrandInfo", ".content-brand-info", ".brand-block2", false);
	addRow("#new-handle-url", "#edit-handle-url", "#blogSave", "#HandleUrl", ".micro-table", ".micro-block", false);
...
/*$(window).load(function() {
 //Update mainfram height & remove load mask
 //$('#wrapper').height($(this).height());
 $('.tab-data,  .right-col, .product-block, .micro-block').height($(this).height() - 100);
 $(".thumbnail-block, .inner-holder").height($(this).height() - 260);
 //updateBrandDropDown();
 //jcf.replaceAll();
 });*/
...
function ajaxCount() {
	...
	$(document).ajaxStop(function() {
		...
		//console.log("\najax Stop	-	 count:" + counter + "\n");
		if (counter == 0) {
			jQuery('.open-close tr').openClose("destroy");
			style();
			$(".load").fadeOut(500);
			ajaxFlag = false;
...
function getAccountInfo() {
	...
	$.ajax({
		...
		if ( typeof json === "object") {
			...
			$(".accountID").html(str);
			currentAccountID = $(".accountID").val();
...
function getCampaignImage() {
	$.getJSON("http://" + global_domain + "/php/Brands.php?method=CampaignImage&brand=" + getBrand(), function(data) {
		//var items = [];
		compaignImage = data;
	});
...
function is_containImage(imageID) {
	...
	$.each(compaignImage, function(key, val) {
		...
		if ($.inArray(imageID, val) > -1) {
			...
			pushCampaginImage_list(key);
		}
	});
}

function pushCampaginImage_list(campaignID) {
	if (compaignImage_list.indexOf(campaignID) == -1) {
		compaignImage_list.push(campaignID);
	}
}

function removeCampaginImage_list() {
	return compaignImage_list.shift();
}

function getCampaginImage_list() {
	return compaignImage_list;
}

function clear_CampaginImage_list() {
	compaignImage_list.length = 0;
}
 ...
 */
function BrandTemplate2(name, BrandID, BrandPhil, BrandInfo, Description, MicroBlog) {
	var row = $("<tr/>").attr("value", BrandID);
	var BrandName = $("<td/>").html(name).appendTo(row);
	var edit = $("<td/>").html($("<a/>").addClass("brandEdit edit Newlightbox").attr("href", "#popup2").html("Edit")).appendTo(row);
	var input = $("<input/>").addClass("brandPost").attr("name", "Name").attr("type", "hidden").val(name).appendTo(row);
	var input = $("<input/>").addClass("BrandPhilosophy").attr("name", "BrandPhilosophy").attr("type", "hidden").val(BrandPhil).appendTo(row);
	var input = $("<input/>").addClass("MicroBlogs").attr("name", "MicroBlogs").attr("type", "hidden").val(MicroBlog).appendTo(row);
	var input = $("<input/>").addClass("BrandInfo").attr("name", "BrandInfo").attr("type", "hidden").val(BrandInfo).appendTo(row);
	var input = $("<input/>").addClass("Description").attr("name", "Description").attr("type", "hidden").val(Description).appendTo(row);
	var input = $("<input/>").addClass("brandID").attr("name", "BrandID").attr("type", "hidden").val(BrandID).appendTo(row);
	return row;
}

function getData(element, method, type, BrandID) {
	var url = "http://" + global_domain + "/php/Brands.php?method=" + method + "&accountID=" + currentAccountID + "&brand=" + BrandID;
	...
	$.ajax({
		...
		reset(element);
		//clean container
		//console.log("data length = ", $.trim(data).length);
		...
		if (element == ".brand-table") {
			...
			if ( typeof json == "object") {
				$.each(json, function(index, key) {
					...
					//console.log("Key: "+ key);
					$(element).append(BrandTemplate2(key["Name"], key["BrandID"], key["BrandPhilosophy"], key["BrandInfo"], key["Description"], key["MicroBlogs"]));
				});
				getAllBrandInfo();
				lightboxReload();
				initCustomForms();
			}
		} else if ($.trim(data).length > 1) {
			if (type == 0) {
				$(element).html(data);
			} else {
				$.when($(element).append(data)).done(function() {
					if (method == "product") {
						...
						if (element == ".product-table") {
							...
							$(".brand").val(currentBrandID).change();
							//getProductCollection();
						}
						//updateBrandDropDown();
						lightboxReload();
						...
						//$(".accountBrand").change();
						//onChangeSelectBrand();
						//console.log("current Brands = ", currentBrandsID);
...
function MainControl() {
	...
	if ( typeof currentPage === 'undefined') {
		...
		console.log("2.CurrentAccount		= ", currentAccountID);
		console.log("3.CurrentBrand			= " + getBrand() + "\n");
		switch(currentPage) {
		case "#home_page":
			//getData(".accountBrand", "brand");
			$("#allpage").show();
		...
		case "#admin_page":
			...
			//console.log("switch currentPage=", currentPage);
			//getData(".accountBrand", "brand");
			break;
		case "#brand_page":
			//console.log("Hide");
			...
			$(".top-bar").css("background", " url(../main_frame/images/bg-nav.png) no-repeat 50% bottom");
			getData(".brand-table", "brand", "manager");
			//addRow("#addBrand", ".brandEdit", "#BrandNameBtn", "#BrandName", ".brand-table", ".brandPost");
			//post("postBrand", ".brand-block form.Post, .brand-block2 form, .micro-block");
		...
		case "#clientele_page":
			...
			break;
		case "#campaign_page":
			//$.when(getData(".accountBrand", "brand")).done(function() {
			getData(".campaign", "campaign", null, getBrand());
			getData(".library-table", "campaign", "manager", getBrand());
			//});
			//post("postCampaign", "form.compaign-form");
			break;
		case "#content_page":
			...
			getCampaignImage();
			getData(".library-table", "campaign", "manager", getBrand());
			getData(".product", "product", null, getBrand());
			getImage(currentBrandID);
		...
		case "#architecture_page":
			...
			//setBrand($(".accountBrand").val());
			//getData(".collection", "collection");
			getData(".product-table", "product", "manager", getBrand());
			//if($(".brand-table tr").length <2){
			getData(".brand-table", "brand", "manager");
			//}
			//getAllBrandInfo();
			//});
			//addRow("#new-product", ".productEdit", "#ProductNameBtn", "#productName", ".product-table", ".productCollection");
			//post("postProduct", ".product-block form.Post");
			//post("postBrand", ".brand-block form.Post, .brand-block2 form, .micro-block");
			//initTabs();
			break;
...
function post(method, target) {
	jQuery(target).bind('submit', function(event) {
		...
		$.ajax({
			type : "POST",
			url : "http://" + global_domain + "/php/Brands.php?method=" + method + "&accountID=" + currentAccountID + "&brand=" + currentBrandID,
			data : json
		}).done(function(respones) {
			//console.log("return value from post =", respones);
			alert("Done!");
			...
			case "postBrand":
				...
				getData(".brand-table", "brand", "manager");
				getData(".accountBrand", "brand");
				//$('.account-form .accountBrand').find('option').clone().appendTo('.brand');
				break;
			case "postProduct":
				console.log("postProduct");
				getData(".product", "product", null, currentBrandID);
				getData(".product-table", "product", "manager", currentBrandID);
				break;
			case "postCampaign":
				console.log("postCampaign");
				getData(".campaign", "campaign", null, currentBrandID);
				getData(".library-table", "campaign", "manager", currentBrandID);
				break;
...
function setBrand(brandID) {
	//console.log("\n\nSetBrandID\nBefore: currentBrandID = ", currentBrandID);
	currentBrandID = brandID;
	//console.log("\nAfter: currentBrandID = ", currentBrandID + "\n");
}

function getBrand() {
	//console.log("\nGet currentBrandID = ", currentBrandID + "\n\n");
	return currentBrandID;
}

function onChangeSelect() {
	onChangeSelectBrand();
	onChangeSelectAccount();
}

function onChangeSelectBrand() {
	var clicked = false;
	$('.accountBrand').on('change', function() {
		//console.log("Change - AccountBrand = ", $(this).val());
		setBrand($(".accountBrand").val());
		MainControl();
	});
}
...
function onChangeSelectAccount() {
 ...
 *
 */
function pushUpdate(updateID) {
	if (updated_array.indexOf(updateID) == -1) {
		updated_array.push(updateID);
	}
}

function removeUpdate() {
	return updated_array.shift();
}
...
function clear_update() {
	updated_array.length = 0;
}

function initSelect() {
	currentAccountID = $(".accountID").val();
	currentBrandID = $(".accountBrand").val();
	$(".brand-block2 input.brandID").val(currentBrandID);
}

function microBlog(handle) {
	var row = $("<tr/>");
	...
	//$("<a/>")..attr("href","http://twitter.com/handle").attr("onclick", "window.open(this.href,'name','width=600,height=400')").html("Test");

	var input = $("<td/>").html($("<input/>").attr("type", "checkbox")).appendTo(row);
	return row;
}

function BrandTemplate(BrandName, unique) {
...
function ProductTemplate(ProductName) {
	var row = $("<tr/>");
 ...
 *
 */
function addRow(addBtn, editBtn, popupSaveBtn, popupInput, table, form, flag) {
	var clicked = false;
	var NameData, rowData;
	var inputName = $(popupInput);
	var count = 0;
	if (flag) {
		//reset button
		$(document).on("click", ".cancelReset, #fancybox-close, #fancybox-overlay", function() {
			console.log("closed");
			inputName.val("");
			clicked = false;
		});
	}
	if (form != ".brand-block2") {
		//add button
	...
	$(document).on("click", editBtn, function() {
		...
		if (form == ".brand-block2") {
			if ($(this).html() == "Edit") {
				NameData = $(this).prev().find("p");
				console.log("This Name = ", NameData);
				inputName.val(NameData.html());
				console.log("This input" + inputName + ", Name = " + inputName.val());
			} else {
				console.log("add clicked");
				clicked = true;
			}
		} else {
			NameData = $(this).parent().prev();
			//target td data
			rowData = $(this).parent().parent();
			//target tr data
			inputName.val(NameData.html());
			//target td data to input field
		}
	});
	//popup save button
	$(document).on("click", popupSaveBtn, function() {
		//user clicked add button
		if (clicked) {
			//Brand
			if (table == ".brand-table") {
				pushUpdate("Brand" + count);
				$(table).append(BrandTemplate(inputName.val(), count++));
				lightboxReload();
			}
			//Product
			else if (table == ".product-table") {
				$(table).append(ProductTemplate(inputName.val()));
				appendOptionToProduct();
				initCustomForms();
				lightboxReload();
			} else if (form == ".micro-block") {
				pushUpdate(getBrand());
				$(table).append(microBlog(inputName.val()));
				initCustomForms();
				lightboxReload();
			} else {
				pushUpdate(getBrand());
				$(table).append($("<p/>").html(inputName.val()));
				$(table).parent().find("a").attr("id", "edit-brand-info").html("Edit");
			}

		}//user clicked edit button
		else if (NameData != null) {
			//console.log("Edit Text");
			//console.log("Input = ", inputName.val());
			NameData.html(inputName.val());
			//console.log("Edited element = ", NameData.html());
			if (form != ".brand-block2") {
				$(rowData).find(form).val(inputName.val());
				//input to target tr
			}

			if (table == ".brand-table") {
				pushUpdate($(rowData).attr("value"));
			} else if (form == ".brand-block2" || form == ".micro-block") {
				pushUpdate(getBrand());
			}
		} else {
			console.log("problem ");
	...
	$(".product-table tr:gt(0)").each(function() {
		...
		if (value != "") {
			...
			if (target[0] != null) {
				...
				//console.log("before -> ", $(this).find(".collection").val());
				//if($(this).find(".collection").val() !=value){
				$(this).find(".collection").val(value);
				$(this).find(".collection").trigger("change");
				//console.log("After -> ", $(this).find(".collection").val());
			}
			//}
		}
	});
...
/*function updateBrandDropDown() {
 $(".brand").val(currentBrandID);
 $(".brand-block2 input.brandID").val(currentBrandID);
 $(".brand").trigger("change");
 //$("input.brandName").val(currentBrandName);
 //$(".jcf-select-brand .jcf-select-text").html($( ".brand option[value='"+currentBrandID+"']").html());
 }*/
...
function getAllBrandInfo() {
	...
	$(".content-brand-philosophy").parent().find("a").html("New");
	$(".content-brand-info").find("p").remove();
	$(".content-brand-info").parent().find("a").html("New");
	$(".micro-table tr:gt(0)").remove();

	$(".brand-table tr:gt(0)").each(function() {
		//console.log("This Brand = ", $(this).find("input.brandPost").val());
		//console.log("This BrandID = ", $(this).attr("value"));
		//console.log("current BrandID = ", getBrand());
		if ($(this).attr("value") == getBrand()) {
			//console.log("BrandPhilosophy", $(this).find("input.BrandPhilosophy").val());
			//console.log("BrandInfo", $(this).find("input.BrandInfo").val());
			//console.log("Description", $(this).find("input.Description").val());
			...
			if (BrandPhilosophy != "") {
				...
				$(".content-brand-philosophy").parent().find("a").attr("id", "edit-brand-philosophy").html("Edit");
			}
			if (BrandInfo != "") {
				$(".content-brand-info").append($("<p/>").html(BrandInfo));
				$(".content-brand-info").parent().find("a").attr("id", "edit-brand-info").html("Edit");
			}

			//$(".brand-block2 form").find('textarea[name=BrandPhilosophy]').val(BrandPhilosophy);
			//$(".brand-block2 form").find('input[name=BrandInfo]').val(BrandInfo);
			//$(".brand-block2 form").find('textarea[name=Description]').val(Description);
			...
			if (MicroBlogs != "") {
				if (MicroBlogs.indexOf(".com/") > -1) {
					...
					//console.log("This MicroBlogs  append = ", microBlog(trimed));
					$(".micro-table").append(microBlog(trimed));
					//$("#popup3 iframe").attr("src", MicroBlogs);
				} else {
					//console.log("This MicroBlogs  append = ", microBlog(MicroBlogs));
					$(".micro-table").append(microBlog(MicroBlogs));
				}
				//console.log("MicroBlogs = ", MicroBlogs);
				//$(".micro-block tr:eq(1)").find('.handle').val(trimed);
			}
		}
	});
}
 ...
 */
function logout() {
	$("a.logout").attr("href", "http://" + global_domain + "/php/logout.php?id=" + cookieID);
}
...
function clickCheckbox() {
	$.each(getCampaginImage_list(), function(index, value) {
...
function createCampaignImage() {
	...
	//var checkCampaign = $(".aside .add").find("input[type=checkbox]:checked");
	var selectedImage = $(".thumbnail-block").find("li.selected");
	if (selectedImage.length == 1) {
		var current = getClickedCamapign();
		var old = getCampaginImage_list();
		var create = [];
		var remove = [];
	...
	/*$(checkCampaign).each(function() {
	 var campaign = $(this);
	 $(selectedImage).each(function() {
	 var image = $(this);
	 json["ImageID"] = image.find("input#ImageID").val();
	 json["CampaignID"] = campaign.attr("id");
	 tmp[i++] = json;
	 json = {};
	 });
	 });
...
function createImageJson(file) {
	...
	arr["Name"] = url.substring(url.indexOf(".net/") + 5, url.length);
 ...
 *
 */
function ConvertFormToJSON(form, single) {
	var array = [];
	var json = {};
	var temp = {};
	var i = 0;
	...
	if (single) {
		var temp1;
		//$.each(getUpdate(), function(index,value){
		//if (getUpdate().length > 0) {
		//console.log("with update")
		while (getUpdate().length > 0) {
			var brandID = removeUpdate();
			console.log("\nbrandID : ", brandID);
			$('.brand-table').find('tr').each(function() {
...
function getBrandInfo() {
	$(".brand-table tr").each(function() {
		if ($(this).attr("value") == currentBrandID) {
...
;( function($) {
		...
		OpenClose.prototype = {
			...
			showSlide : function() {
				...
				if (self.options.addClassBeforeAnimation) {
					console.log("Show slide	-	addClassBeforeAnimation");
				...
				console.log("Show slide	-		toggleEffects");
		...
		var toggleEffects = {
			slide : {
				show : function(o) {
					console.log("\nShow 	-	o.speed: "+o.speed, +" , o.complete: "+o.complete);
					o.box.stop(true).hide().slideDown(o.speed, o.complete);
				},
				hide : function(o) {
					console.log("\nHide		-	o.speed: "+o.speed, +" , o.complete: "+o.complete);
