	...
	public function brandTemplate($data) {
		...
		foreach ($data as $k => $v) {
			foreach ($v as $a => $key) {
				...
				"<input type='hidden' name='BrandPhilosophy' value='" . addslashes($key['BrandPhilosophy']) . "' class='BrandPhilosophy'/>" . 
				"<input type='hidden' name='MicroBlogs' value='" . $key['MicroBlogs'] . "' class='MicroBlogs'/>" . 
				"<input type='hidden' name='BrandInfo' value='" . $key['BrandInfo'] . "' class='BrandInfo'/>" . 
				"</tr>";
				echo $str;
			}
		}
