...
function CollectionTemplate(CollectionName, id) {
	var li = $("<li/>");
	var a = $("<a/>").attr("id", id).attr("href", "collection_content?collection=" + id).html(CollectionName).appendTo(li);
	return li;
}
	...
	$.getJSON(domain + "/php/api_collection.php?method=search", function(data) {
		...
		$("#search").autocomplete({
			...
			select : function(event, ui) {
				window.location.href ="../brand_page/brand_content?brand="+ui.item.id;
			}
		});
	});
