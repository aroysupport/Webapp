...
$(function() {
	...
	imageload();
	//---------init style--------
	init();//use many place cannot delete
	//---------init style-------
	initUploadFiles();//use when browse img upload show info in auto input box
	//postCampaign();
	onChangeSelect();
	initSelect();
	logout();
...
function onChangeSelect() {
	$('.accountBrand').on('change', function() {
		...
		getTest(".library-table", "newCampaign", "manager", currentBrandID);
		//updateBrandDropDown();
	});
	$(' .accountID').on('change', function() {
		currentAccountID = $(this).val();
		getTest(".collection", "collection");
		getTest(".campaign", "newCampaign");
				...
				if (data.length > 1) {
					$.when($(element).append(data)).done(function() {
						...
						if (method == "product") {
							...
							updateBrandDropDown();
							/*jQuery('tr.library-table').each(function() {
...
function postCampaign(file) {
	//jQuery('form.compaign-form').bind('submit', function(event) {
		...
		$.ajax({
			type : "POST",
			url : "http://" + global_domain + "/php/Brands.php?method=postCampaign&id=" + cookieID,
...
function initUploadFiles() {
	...
	$('.upload-form input[type=submit], form.compaign-form input[type=submit]').addClass('btn btn-primary').prop('disabled', true).on('click', function(event) {
		event.preventDefault();
...
ManePage.remoteIn = {
	...
	login: function() {
		...
		$("#btn-remote-login").click(function() {
			self._login();
		});
	},
